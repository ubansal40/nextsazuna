 ...
 * edit rather than a deploy. The design spec's eight sections map one-to-one
 * onto the block's eight entries.
 *
...
export async function getHomepageBlocks(): Promise<HomeBlock[]> {
  const block = await getContentBlock<{ blocks?: unknown }>("homepage_layout");
